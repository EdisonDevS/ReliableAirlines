...
AS
	DECLARE @primeraClase INT=(SELECT SUM(CAST(VUELOS.vlrPrimeraClase AS INT)) FROM VUELOS INNER JOIN TIQUETE ON 
								...
								TIQUETE.estadoDeReserva!=1 AND MONTH(TIQUETE.fechaReserva)=@mes AND YEAR(TIQUETE.fechaReserva)=@año)
	...
	DECLARE @externos INT=(SELECT SUM(CAST(MOVIMIENTOS_FINANCIEROS_EXTERNOS.valor AS INT)) FROM MOVIMIENTOS_FINANCIEROS_EXTERNOS 
							...
							YEAR(MOVIMIENTOS_FINANCIEROS_EXTERNOS.fecha)=@año)
...
AS
	DECLARE @primeraClase INT=(SELECT SUM(CAST(VUELOS.vlrPrimeraClase AS INT)) FROM VUELOS INNER JOIN TIQUETE ON 
								...
								AND MONTH(TIQUETE.fechaReserva)=@mes AND YEAR(TIQUETE.fechaReserva)=@año)
...
AS
	...
	WHERE TIQUETE.estadoDeReserva=2 AND RUTAS.idRuta=@idRuta AND YEAR(VUELOS.salida)=@año AND MONTH(VUELOS.salida)=@mes
GO 
...
AS
	...
	CIUDADES.id=@idCiudad AND YEAR(VUELOS.salida)=@año AND MONTH(VUELOS.salida)=@mes
GO
...
INSERT INTO USUARIO VALUES('1','NN','ROOT','TOOR','1','USUARIO','ADMINISTRADOR','1999/1/1','ADMIN@ADMIN.COM','123546789')
GO